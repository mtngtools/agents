...
- **Prefer the repo's own no-container gate over a filter you invent.** Some repos ship a second entry point beside the gate — in `mtng-dotnet-mono` it is `./eng/stable/check-fast.sh` beside `check.sh`. Look for one before improvising: `AGENTS.md`, the gate script's own README, the `eng/` directory. Using it matters for three reasons beyond convenience. The exclusion is then **the repo's decision rather than your guess**, so you cannot accidentally skip more or less than it intends. It carries the repo's environment posture — telemetry opt-outs, configuration defaults — which a hand-rolled `dotnet test` loop silently does not. And it is **reproducible by the author**, who can run the identical command against your report. Where no such entry point exists, the hand-rolled filter above is correct; say which you did.
- **Counts are not comparable across rounds unless the run was.** Two legitimate runs of the same gate differ by a whole suite — a fast run and a full one are not the same denominator. When your total moves between rounds, say which of the two moved it *before* a reader reads a drop as tests disappearing. If the earlier round ran wider than yours, that difference belongs in **Test Coverage**, not left for someone to reconcile.
- **A fast gate is not enough when the diff reaches what it skips.** The container suites are exempt because ordinary work cannot break them — not because they never matter. Where the diff touches the projects those suites actually cover, the excluded run is no longer sufficient evidence on its own, and `gh pr checks <n>` becomes the load-bearing part of the verdict rather than a footnote. Say so explicitly in that case; do not let a green fast run stand in for coverage it never had.
