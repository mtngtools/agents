...
**Wayfinder decision issues are inputs, not the build backlog.** They record *why* a decision was made and stay closed as the rationale trail. Nothing is built from them directly — what gets built is the **spec**, sliced fresh by `/to-tickets` into its own implementation tickets. Never hand a wayfinder decision issue to `/implement`.

## Process

### 1. Load the map and its decisions

Resolve the map from the argument (issue URL or number). If none was given, find the open issue labelled `wayfinder:map`. Read its body — **Destination** and **Decisions so far**. For every decision listed, fetch its child ticket and read the `## Answer`.
...
Then report to the user: what landed where, and that `/specs-to-tickets <map>` is the next step.
...
`/wayfinder` → GitHub **decision** issues (closed, rationale) → **`/decisions-to-specs`** → repo **spec files** (+ `## Specs settled` on the map) → `/specs-to-tickets` → `/to-tickets` → GitHub **implementation** tickets (fresh, distinct from the decision issues) → `/implement`.
