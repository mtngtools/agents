---
name: 75-concise
description: Cut text to about 75% of its length, keeping all meaning.
argument-hint: "text or file to reduce"
metadata:
  type: command
  invocation: skill-callable
  applies-to: [writing, documentation, reduction]
---

# 75-concise

**Target: 75% of the original length.**

Apply the [reduction method](../concise-copy/reduce.md). Meaning is the floor; the target is only a ceiling — a light trim, so lose nothing.
